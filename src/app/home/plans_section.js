...
export default function PlansSection() {
  ...
  function togglePlans() {
    if (selectedPlanType === "domestic") {
      setSelectedPlanType("commercial");
    } else {
      setSelectedPlanType("domestic");
    }
  }
      ...
      <div className="flex flex-col relative  w-full">
        <div className="flex flex-col gap-5 md:gap-6 h-full text-[var(--dark-2)] items-center text-center justify-center px-5 md:px-6 pb-28 md:pt-10 z-10 relative">
          ...
          <div className="flex flex-col mt-6 items-center gap-2">
            ...
            <div
              className={`flex border border-gray-400/40 rounded-lg p-2 items-center justify-center`}
            >
              <button
                onClick={togglePlans}
                className={` px-5 py-1  tracking-wide uppercase ${
        ...
        <div className="w-full group/plan overflow-hidden self-center max-w-7xl -mb-40 -mt-16 relative z-10">
          <button
            onClick={goPrev}
            className="hidden md:block absolute z-30 left-2 opacity-0 -translate-x-full group-hover/plan:opacity-100 group-hover/plan:translate-x-0 top-1/2 -translate-y-1/2 p-5
          ...
          <button
            onClick={goNext}
            className="hidden md:block absolute z-30 right-2 opacity-0 translate-x-full group-hover/plan:opacity-100 group-hover/plan:translate-x-0 top-1/2 -translate-y-1/2 p-5
